...
/******************************************************************************/
enum keyTypeEnum { NO_KEY_TYPE, RSA_KEY_TYPE, ECC_KEY_TYPE };
...
static WC_INLINE int PasswordCallBack(char *passwd, int sz, int rwflag,
                                      void *userdata) {
    (void)
        rwflag; /* we aren't going to implement writes differently than reads*/
    (void)userdata;
...
static bool naked_PEM_to_PEM(const char *in, char **pem, int type) {
    ...
    if ((CERT_TYPE != type) && (PRIVATEKEY_TYPE != type) &&
        (ECC_PRIVATEKEY_TYPE != type) && (CERTREQ_TYPE != type)) {
...
static bool PEMx509List_add(PEMx509List *pList, WOLFSSL_X509 *pCert) {
    bool bResult = false;
    if (pList && pCert) {
        WOLFSSL_X509 **temp = realloc(pList->certs, (1 + pList->item_count) *
                                                        sizeof(WOLFSSL_X509 *));
        if (temp) {
            pList->certs = temp;
...
/*         - failure : false                                                  */
/*                                                                            */
static bool PemInventoryItem_populate(PemInventoryItem *pem,
                                      WOLFSSL_X509 *cert) {
    bool bResult = false;
    char *pThumb = NULL;
        ...
        if (certBaseId == keyBaseId) {
            ...
            case EVP_PKEY_EC:
                ...
                if (ecPriv && ecCert) {
                    ...
                    /* octets for y.                                          */
                    /*                                                        */
                    privPubBytes = wolfSSL_EC_POINT_point2hex(
                        privGroup, privPoint, POINT_CONVERSION_UNCOMPRESSED,
                        NULL);
                    ...
                    certPubBytes = wolfSSL_EC_POINT_point2hex(
                        certGroup, certPoint, POINT_CONVERSION_UNCOMPRESSED,
                        NULL);
...
static unsigned long write_key_bio(WOLFSSL_BIO *pBio, const char *password,
    ...
    const WOLFSSL_EVP_CIPHER *tmpCiph = (password && strcmp(password, "") != 0)
    ...
    if (NULL == pkey) /* We want to save the keyPair since no key was passed*/
    {
        ...
        errNum = wolfSSL_PEM_write_bio_PKCS8PrivateKey(
            pBio, pPrivateKey, tmpCiph, NULL, 0, PasswordCallBack,
            (char *)tmpPass);
        if (0 < errNum) {
        ...
        errNum = wolfSSL_PEM_write_bio_PKCS8PrivateKey(
            pBio, pkey, tmpCiph, NULL, 0, PasswordCallBack, (char *)tmpPass);
        if (0 < errNum) {
            log_verbose("%s::%s(%d) : Key written to BIO", LOG_INF);
            errNum = 0;
        } else {
    ...
    *ppKeyList = PrivKeyList_new();
    ...
    while (wolfSSL_PEM_read(fp, &name, &header, &pData, &length)) {
        ...
        if ((strcasecmp(name, "CERTIFICATE") == 0)) {
            ...
            pKey = wolfSSL_d2i_PKCS8PrivateKey_bio(
                pKeyBio, &pKey, PasswordCallBack, (void *)tmpPass);
...
static int get_inventory(const char *path, const char *password,
    ...
    *ppPemList = PemInventoryList_new();
    ...
    if ((NULL == (*ppPemList)) || (NULL == x509array) || (NULL == keyList)) {
        log_error("%s::%s(%d) : Out of memory", LOG_INF);
        ret = -1;
        goto cleanup;
    }
    ...
    while (PEM_read(fp, &name, &header, &data, &length)) {
        ...
        if ((strcmp(name, "CERTIFICATE") == 0) &&
            ...
            key = wolfSSL_d2i_PKCS8PrivateKey_bio(
                pKeyBio, &key, PasswordCallBack,
            ...
            if (NULL != key) {
                ...
                log_error(
                    "%s::%s(%d) : Unable to decrypt private key: %s = %ld",
...
bool ssl_generate_rsa_keypair(int keySize) {
    ...
    if (0 == errNum) {
        ...
        pPrivateKey =
            wolfSSL_d2i_PrivateKey(EVP_PKEY_RSA, &pPrivateKey, ppDer, derSz);
        if (pPrivateKey) {
...
bool ssl_generate_ecc_keypair(int keySize) {
    ...
    if (errNum != 0) {
        log_error("%s::%s(%d) : Error generating ecc key. code = %ld", LOG_INF,
                  errNum);
        goto fail_cleanup;
    }
    ...
    ppDer = (const unsigned char **)&pDer;
    pPrivateKey =
        wolfSSL_d2i_PrivateKey(EVP_PKEY_EC, &pPrivateKey, ppDer, derSz);
    if (pPrivateKey) {
...
char *ssl_generate_csr(const char *asciiSubject, size_t *csrLen,
    ...
    if (ret != 0) {
        ...
        append_linef(pMessage, "%s::%s(%d) : Init cert failed %d", LOG_INF,
                     ret);
        goto cleanup;
    }
    req.version = 1;
    ...
    case ECC_KEY_TYPE:
        ...
        ret = wc_SignCert(req.bodySz, req.sigType, pDer, MAX_CSR_SIZE, NULL,
                          &eccKey, &rng);
    ...
    case RSA_KEY_TYPE:
        ...
        ret = wc_SignCert(req.bodySz, req.sigType, pDer, MAX_CSR_SIZE, &rsaKey,
                          NULL, &rng);
        log_trace("%s::%s(%d) : Successfully exited signing function", LOG_INF);
        break;
    default:
...
unsigned long ssl_save_cert_key(const char *storePath, const char *keyPath,
    ...
    if (err) {
        wolfSSL_ERR_error_string(err, errBuf);
        log_error("%s::%s(%d) : Unable to write key to BIO: %ld - %s", LOG_INF,
                  err, errBuf);
        append_linef(pMessage, "Unable to write key to BIO: %ld - %s", err,
                     errBuf);
        goto cleanup;
    }
...
bool ssl_remove_cert_from_store(const char *storePath, const char *searchThumb,
    ...
    if (certFound) {
        ...
        for (int j = 0; pemList->item_count > j; j++) {
            if (i != j) {
                ...
                if (pem)
                    free(pem);
                pem = NULL;
                if (!naked_PEM_to_PEM(pemList->items[j]->cert, &pem,
                                      CERT_TYPE)) {
        ...
        if (keyArray) {
            for (int k = 0; keyArray->key_count > k; k++) {
                if (!is_cert_key_match(pemX509Array->certs[i],
                    ...
                    log_trace("%s::%s(%d) : Writing key #%d to BIO", LOG_INF,
                              k);
        ...
        if (0 != ret) {
            char *errStr = strerror(ret);
            log_error("%s::%s(%d) : Unable to write BIO at %s: %s", LOG_INF,
                      storePath, errStr);
            goto cleanup;
        }
...
bool ssl_is_cert_active(char *certFile) {
    ...
    do {
        ...
        if (start_date && end_date) {
            log_verbose(
                "%s::%s(%d) : Certificate is valid from "
                "20%c%c-%c%c-%c%c %c%c:%c%c:%c%c GMT to "
                "20%c%c-%c%c-%c%c %c%c:%c%c:%c%c GMT",
                LOG_INF, (char)start_date[2], (char)start_date[3],
                (char)start_date[4], (char)start_date[5], (char)start_date[6],
                (char)start_date[7], (char)start_date[8], (char)start_date[9],
                (char)start_date[10], (char)start_date[11],
                (char)start_date[12], (char)start_date[13], (char)end_date[2],
                (char)end_date[3], (char)end_date[4], (char)end_date[5],
                (char)end_date[6], (char)end_date[7], (char)end_date[8],
                (char)end_date[9], (char)end_date[10], (char)end_date[11],
...
bool ssl_init(void) {
    ...
    if (WOLFSSL_SUCCESS != errNum) {
        log_trace("%s::%s(%d) : wolfSSL_Init failed with code = %d", LOG_INF,
                  errNum);
        return false;
    }
...
bool ssl_cleanup(void) {
    ...
    if (WOLFSSL_SUCCESS != errNum) {
        log_error("%s::%s(%d) : wolfSSL_Cleanup failed with code = %d", LOG_INF,
                  errNum);
        bResult = false;
    }
    return bResult;
