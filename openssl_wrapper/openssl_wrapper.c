...
static bool pemify(char **pToB64, const char *header, const char *footer) {
    ...
    log_debug("%s::%s(%d) : The size of the header + footer + data leads to a "
              "string of size %lu bytes "
              "the buffer is of size %d",
              LOG_INF, tempSize, MAX_CERT_SIZE);
    if ((size_t)MAX_CERT_SIZE < (tempSize + 1)) {
        log_error("%s::%s(%d) : The size of the header + footer + data is too "
                  "large for the buffer, which is "
                  "sized at %d bytes",
                  LOG_INF, MAX_CERT_SIZE);
        return false;
    }

    do {
        log_trace("%s::%s(%d) : Writing to result variable PEM header = %s",
                  LOG_INF, header);
        tempSize = strlen(header);
        memcpy(result, header, tempSize); // don't copy \0
        outputArrayPtr += tempSize;
        log_trace(
            "%s::%s(%d) : Successfully wrote PEM header to result variable",
            LOG_INF);

        log_trace("%s::%s(%d) : Attempting to parse the input string:\n%s",
                  LOG_INF, *pToB64);
        log_trace("%s::%s(%d) : The input string is %lu characters long",
                  LOG_INF, base64Length);
        inputArrayPtr = 0;
        while (base64Length > inputArrayPtr) {
            ...
            log_trace("%s::%s(%d) : Attempting to write %lu bytes from input "
                      "array at location %lu into output "
                      "array at location %lu",
                      LOG_INF, lenToWrite, inputArrayPtr, outputArrayPtr);
            memcpy(result + outputArrayPtr, *pToB64 + inputArrayPtr,
                   lenToWrite);
            inputArrayPtr += lenToWrite; // Increment the pointer into the input
                                         // array by 64. 0,64,128,192,etc.
            outputArrayPtr +=
                lenToWrite; // Increment the pointer into the result array
            log_trace("%s::%s(%d) : Attempting to write a carriage return into "
                      "output array at location %lu",
                      LOG_INF, outputArrayPtr);
            result[outputArrayPtr] = '\n';
            outputArrayPtr++;
            result[outputArrayPtr] =
                '\0'; // Temporarily end this string so we can print it.
            if (is_log_debug()) {
                log_debug("%s::%s(%d) : Current result is %lu long", LOG_INF,
                          strlen(result));
            }
        }
        worked = true;
    ...
    if (worked) {
        log_trace("%s::%s(%d) : Writing to result variable PEM footer = %s",
                  LOG_INF, header);
        ...
        log_trace(
            "%s::%s(%d) : Successfully wrote PEM footer to result variable",
            LOG_INF);
    }
        ...
        if (*pToB64) {
            ...
            if (NULL == temp) {
                ...
                *pToB64 = NULL;
                ...
                log_trace("%s::%s(%d) : Successfully reallocated memory",
                          LOG_INF);
                *pToB64 = temp;
    ...
    do {
        ...
        if (PEM_read(fp, &name, &header, &data, &length)) {
            ...
            if (0 == (strcmp(name, "CERTIFICATE"))) {
                log_trace("%s::%s(%d) : Found a certificate, continuing",
                          LOG_INF);
                if ((d2i_X509(&x509, &tempData, length))) {
                    log_debug("%s::%s(%d) : Successfully converted PEM into an "
                              "x509 cert",
                              LOG_INF);
                } else {
                    log_error("%s::%s(%d) : Failed to convert read certificate "
                              "into an x509 cert",
                              LOG_INF);
                    if (name)
                        OPENSSL_free(name);
                ...
                log_error("%s::%s(%d) : Error, file is not a certificate",
                          LOG_INF);
                if (name)
                    OPENSSL_free(name);
                if (header)
        ...
        if (bio) {
            if (1 == ASN1_TIME_print(bio, date)) {
                ...
                if (0 < written) {
                    ...
                    log_debug("%s::%s(%d) : converted string = %s", LOG_INF,
                              buf);
                    if (bio)
                        BIO_free(bio);
                    ...
                    log_error(
                        "%s::%s(%d) : Error converting ASN1 date to string",
                        LOG_INF);
                    if (bio)
                        BIO_free(bio);
                    break;
                ...
                log_error("%s::%s(%d) : ASN1 date format on certificate is bad",
                          LOG_INF);
                if (bio)
                    BIO_free(bio);
                break;
...
static bool PrivKeyList_add(PrivKeyList *list, EVP_PKEY *key) {
    ...
    if (list && key) {
        EVP_PKEY **temp =
            realloc(list->keys, (1 + list->key_count) * sizeof(EVP_PKEY *));
        if (temp) {
            list->keys = temp;
...
static void PEMx509List_free(PEMx509List *pList) {
    ...
    if (0 < pList->item_count) {
        for (int i = 0; pList->item_count > i; i++) {
            log_trace("%s::%s(%d) Freeing cert #%d from PEMx509List", LOG_INF,
                      i);
...
static bool PEMx509List_add(PEMx509List *list, X509 *cert) {
    ...
    if (list && cert) {
        X509 **temp =
            realloc(list->certs, (1 + list->item_count) * sizeof(X509 *));
        if (temp) {
            list->certs = temp;
...
static PemInventoryItem *PemInventoryItem_new() {
    PemInventoryItem *pem =
        (PemInventoryItem *)malloc(sizeof(PemInventoryItem));
    if (pem) {
        pem->cert = NULL;
...
/*         - failure : false                                                  */
/*                                                                            */
static bool PemInventoryList_add(PemInventoryList *list,
                                 PemInventoryItem *item) {
    bool bResult = false;
    if (list && item) {
        PemInventoryItem **temp = realloc(
            list->items, (1 + list->item_count) * sizeof(PemInventoryItem *));
        if (temp) {
            list->items = temp;
            ...
            log_trace("%s::%s(%d) : Added cert with thumbprint %s to local "
                      "inventory",
                      LOG_INF, item->thumbprint_string);
            bResult = true;
        } else {
            log_error("%s::%s(%d) : Out of memory", LOG_INF);
...
static bool PemInventoryItem_populate(PemInventoryItem *pem, X509 *cert) {
    ...
    if (pem && cert) {
        ...
        if (0 < contLen) {
            ...
            if (!pem->cert) {
                log_error("%s::%s(%d) : Failed to base64 encode certificate",
                          LOG_INF);
                return false;
            }
            pem->thumbprint_string = strdup(thumb);
            if (!pem->thumbprint_string) {
                log_error("%s::%s(%d) : Failed to allocate thumbprint string",
                          LOG_INF);
                free(pem->cert);
                pem->cert = NULL;
                return false;
...
static bool is_cert_key_match(X509 *cert, EVP_PKEY *key) {
    ...
    if (cert && key) {
        ...
        if (NULL == certPubKey) {
            log_error("%s::%s(%d) : Failed to get public key from certificate",
                      LOG_INF);
            return false;
        }
        ...
        if (certBaseId == keyBaseId) /* check for keytype match */
        {
            ...
            case EVP_PKEY_EC:
                ...
                if (ecPriv && ecCert) {
                    ...
                    /*                                                        */
                    privPubBytes =
                        EC_POINT_point2hex(privGroup, privPoint,
                                           POINT_CONVERSION_UNCOMPRESSED, NULL);
                    if (NULL == privPubBytes) {
                        log_error("%s::%s(%d) : Failed to convert private key "
                                  "EC_POINT to hex",
                                  LOG_INF);
                        ret = false;
                        break;
                    ...
                    certPubBytes =
                        EC_POINT_point2hex(certGroup, certPoint,
                                           POINT_CONVERSION_UNCOMPRESSED, NULL);
                    if (NULL == certPubBytes) {
                        log_error("%s::%s(%d) : Failed to convert certificate "
                                  "EC_POINT to hex",
                                  LOG_INF);
                ...
                break;
            default:
                log_error("%s::%s(%d) : Unknown algorithm: %d", LOG_INF,
                          certBaseId);
                break;
            }
        }
    ...
    while (!done && !hasError && subInd < subjLen) {
        ...
        case '\\':
            if (sscanf(&subject[subInd], "\\%1[\" #+,;<=>\\]", escaped) == 1) {
                ...
                subInd += 2;
            } else if (sscanf(&subject[subInd], "\\%1x%1x", &hexHi, &hexLo) ==
                       2) {
                if (buf) {
...
static X509_NAME *parse_subject(const char *subject) {
    ...
    if (is_log_debug()) {
        log_debug("%s::%s(%d) : Subject \"%s\" is %ld characters long", LOG_INF,
    ...
    while (!hasError && !endOfSubject) {
        ...
        log_verbose("%s::%s(%d) : Key: \"%s\" is %ld characters long", LOG_INF,
                    strippedKey, strlen(strippedKey));

        /* Now get the value for the key */
        curPtr += (keyLen + 1); /* Advance past the equals character */
        if (*curPtr != '\0') {
            log_trace("%s::%s(%d) : localSubject is now \"%s\"", LOG_INF,
                      curPtr);
...
static unsigned long write_cert_bio(BIO *bio, const char *b64cert) {
    ...
    log_trace("%s::%s(%d) : Decoded certificate into binary structure",
              LOG_INF);
...
static unsigned long write_key_bio(BIO *bio, const char *password,
    ...
    if (NULL == key) {
        ...
        if (PEM_write_bio_PKCS8PrivateKey(bio, keyPair, tmpCiph, NULL, 0, 0,
                                          (char *)tmpPass)) {
        ...
        if (PEM_write_bio_PKCS8PrivateKey(bio, key, tmpCiph, NULL, 0, 0,
                                          (char *)tmpPass)) {
            log_verbose("%s::%s(%d) : Key written to BIO", LOG_INF);
        } else {
            errNum = ERR_peek_last_error();
...
static int get_key_inventory(const char *path, const char *password,
    ...
    *keyList = PrivKeyList_new();
    ...
    if (!fp) {
        ret = errno;
        char *errStr = strerror(errno);
        log_error("%s::%s(%d) : Unable to open store at %s: %s", LOG_INF, path,
                  errStr);
        free(errStr);
        goto cleanup;
    }
...
static int get_inventory(const char *path, const char *password,
    ...
    *pPemList = PemInventoryList_new();
    ...
    if ((NULL == (*pPemList)) || (NULL == x509array) || (NULL == keyList)) {
        log_error("%s::%s(%d) : Out of memory", LOG_INF);
        ret = -1;
        goto cleanup;
    }
...
/*                                                                            */
/*                                                                            */
static EVP_PKEY *genkey_rsa_using_TPM(BIGNUM *exp, RSA *rsa, int keySize,
                                      TPM2_DATA *tpm2Data) {
...
#endif
{
    ...
    if (NULL == keyPair) {
        ...
        unsigned long errNum = ERR_peek_last_error();
        ERR_error_string(errNum, errBuf);
        log_error("%s::%s(%d) : Unable to generate key pair: %s", LOG_INF,
                  errBuf);
        return NULL;
    }

    log_verbose("%s::%s(%d) : Write encrypted BLOB to disk - %s", LOG_INF,
                path);
...
char *ssl_generate_csr(const char *asciiSubject, size_t *csrLen,
    ...
    if (SSL_SUCCESS == result) {
        ...
        if (SSL_SUCCESS == result) {
            ...
            if (0 == result) {
                ...
                log_error("%s::%s(%d) : CSR signing failed with code, "
                          "0x%X = %s",
                          LOG_INF, result, errBuf);
                append_linef(pMessage,
                             "CSR signing failed with code, "
                             "0x%X = %s",
                             result, errBuf);
                csrString = NULL;
            } else {
            ...
            log_error("%s::%s(%d) : CSR set of public key failed with code, "
                      ...
                      LOG_INF, result, errBuf);
            append_linef(pMessage,
                         "CSR set of public key failed with code, "
                         "0x%X = %s",
                         result, errBuf);
            csrString = NULL;
        }
        ...
        log_error("%s::%s(%d) : CSR subject name set failed with code, "
                  ...
                  LOG_INF, result, errBuf);
        append_linef(pMessage,
                     "CSR subject name set failed with code, "
                     "0x%X = %s",
                     result, errBuf);
        csrString = NULL;
    }
    ...
    /*************************************************************************/
    if (SSL_SUCCESS == result) {
        log_verbose("%s::%s(%d) : Encoding the CSR and converting it to a base "
                    "64 encoded string.",
                    LOG_INF);
        ...
        if (csrString) {
            ...
            if (MAX_CSR_SIZE < *csrLen) {
                log_error("%s::%s(%d) : The length of the CSR = %ld which is "
                          "longer than the maximum defined "
                          "length of %d -- Please increase the maximum CSR "
                          "length above %ld and re-compile",
                          LOG_INF, *csrLen, MAX_CSR_SIZE, *csrLen);
                append_linef(pMessage, "CSR length %ld exceeds maximum %d",
                             *csrLen, MAX_CSR_SIZE);
                free(csrString);
                csrString = NULL;
            }
...
unsigned long ssl_save_cert_key(const char *storePath, const char *keyPath,
    ...
    if (!err) {
        ...
        if (err) {
            char *errStr = strerror(err);
            log_error("%s::%s(%d) : Unable to write store at %s: %s", LOG_INF,
                      storePath, errStr);
...
bool ssl_PemInventoryItem_create(struct PemInventoryItem **pem,
    ...
    if (d2i_X509(&cert, tempPtrPtr, certLen)) {
        ...
        if (NULL == cbio) {
            log_error("%s::%s(%d) : cbio is NULL -- error in SSL call to "
                      "BIO_new(BIO_s_mem())",
                      LOG_INF);
            goto exit;
        }
...
bool ssl_remove_cert_from_store(const char *storePath, const char *searchThumb,
    ...
    while ((!certFound) && (0 <= i)) {
        ...
        if (0 ==
            strcasecmp(searchThumb, pemList->items[i]->thumbprint_string)) {
            certFound = true;
        } else {
            i--;
        }
    ...
    if (certFound) {
        ...
        log_trace("%s::%s(%d) : bio returned %s", LOG_INF,
                  (NULL == bio) ? "NULL" : "a memory location");
        /* At this point i points to the pemList & */
        /* PEMx509List of the cert to delete */
        for (int j = 0; pemList->item_count > j; j++) {
            if (i != j) {
                log_trace(
                    "%s::%s(%d) : Attempting to write cert #%d of #%d to bio",
                    LOG_INF, j + 1, (pemList->item_count));
                len = strlen(pemList->items[j]->cert) + 1;
                if (NULL != pem)
                    free(pem);
                pem = calloc(len, sizeof(*pem)); // get some memory to store the
                                                 // eventual PEM file
                if (NULL == pem) {
                    log_error("%s::%s(%d) : Out of memory", LOG_INF);
                    goto cleanup;
                } else {
                    memcpy(pem, pemList->items[j]->cert,
                           len); // Copy the single string
                    if (false == pemify(&pem, x509PEMHeader, x509PEMFooter)) {
                        ...
                        if (0 >= BIO_puts(bio, pem)) {
                            log_error(
                                "%s::%s(%d) : Failed to write PEM into BIO",
                                LOG_INF);
                            goto cleanup;
                        } else {
                            log_trace(
                                "%s::%s(%d) : Successfully wrote PEM into BIO",
                                LOG_INF);
                        }
                    }
                }
            } else {
                log_trace("%s::%s(%d) : Skipping bio write of cert #%d of #%d",
                          LOG_INF, j + 1, (pemList->item_count));
            }
        }
        /**********************************************************************/
        ...
        if (keyPath) {
            ...
            if (0 != ret) {
                log_error("%s::%s(%d) : Error reading keystore %s", LOG_INF,
                          keyPath);
                goto cleanup;
            }
            bio = BIO_new(BIO_s_mem()); /* Get new memory to store the bio */
        ...
        log_error("%s::%s(%d) Cert not found in PEM store %s", LOG_INF,
                  storePath);
        goto cleanup;
    }
...
#define CERT_DATE_BUF_LEN 30
    ...
    do {
        ...
        if (NULL == x509) {
            log_error("%s::%s(%d) : Error getting certificate from file %s",
                      LOG_INF, certFile);
            break;
        }
        ...
        if (!start_date) {
            ...
            if (get_datestring_ASN1(notBeforeString, CERT_DATE_BUF_LEN,
                                    start_date)) {
                log_trace(
                    "%s::%s(%d) : Successfully converted notBeforeDate = %s",
                    LOG_INF, notBeforeString);
            } else {
                log_error("%s::%s(%d) : Error converting notBeforeDate, "
                          "assuming cert is bad",
                          LOG_INF);
                break;
            }
        }
        ...
        if (!end_date) {
            ...
            if (get_datestring_ASN1(notAfterString, CERT_DATE_BUF_LEN,
                                    end_date)) {
                log_trace(
                    "%s::%s(%d) : Successfully converted notAfterString = %s",
                    LOG_INF, notAfterString);
            } else {
                log_error("%s::%s(%d) : Error converting notAfterString, "
                          "assuming cert is bad",
                          LOG_INF);
                break;
            }
        ...
        if (start_date && end_date) {
            log_verbose("%s::%s(%d) : Certificate is valid from %s to %s",
                        LOG_INF, notBeforeString, notAfterString);
        }
        ...
        if ((time_t)-1 == now) {
            log_error("%s::%s(%d) : Failed to convert local time to GMT",
                      LOG_INF);
            break;
        }
        if (is_log_debug()) {
        ...
        if (get_datestring_ASN1(nowASN1string, CERT_DATE_BUF_LEN, now_asn1)) {
            log_trace("%s::%s(%d) : Successfully converted nowASN1string = %s",
                      LOG_INF, nowASN1string);
        } else {
            log_warn(
                "%s::%s(%d) : Couldn't decode now_asn1; not fatal, continuing",
                LOG_INF);
        }

        int day = 0;
        int sec = 0;
        log_trace("%s::%s(%d) : Comparing start times FROM = %s and TO = %s",
                  LOG_INF, nowASN1string, notBeforeString);
        ASN1_TIME_diff(&day, &sec, now_asn1, start_date);
        log_debug("%s::%s(%d) : day = %d    sec = %d", LOG_INF, day, sec);
        if ((0 < day) || (0 < sec)) {
            log_error("%s::%s(%d) : Error certificate is NOT active yet",
                      LOG_INF);
            break;
        }

        log_trace("%s::%s(%d) : Comparing end times FROM = %s and TO = %s",
                  LOG_INF, nowASN1string, notAfterString);
