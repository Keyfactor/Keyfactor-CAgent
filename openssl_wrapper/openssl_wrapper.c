...
static bool pemify(char **pToB64, const char *header, const char *footer)
{
    ...
    //don 't copy \0
        ...
        inputArrayPtr += lenToWrite;
            //Increment the pointer into the input array by 64. 0, 64, 128, 192, etc.
        outputArrayPtr += lenToWrite;
            //Increment the pointer into the result array 
        ...
        outputArrayPtr++;
        result[outputArrayPtr] = '\0';
            //Temporarily end this string so we can print it.
    ...
    memcpy(result + outputArrayPtr, footer, tempSize);
    //don 't copy \0
    outputArrayPtr += tempSize;
    result[outputArrayPtr] = '\0';
    //Now add the \ 0 explicitly 
...
static bool is_cert_key_match(X509 * cert, EVP_PKEY * key) 
{
    ...
    privGroup = EC_KEY_get0_group(ecPriv);  /* get EC_GROUP */
                    ...
                    /* An EC_POINT represents a point on the EC_GROUP's
                          * curve. */ 
                         /* */ 
                    ...
                    /* used in certificates and TLS records): only the
                          * content */ 
                    /* octets are present, the OCTET STRING tag and length
                          * are */ 
                         /* not included.                                          */ 
                    ...
                    /* terminated character string where each character is
                          * one */ 
 ...
 /* */ 
/* If an ascii escaped hex value is encontered it parses the value            */
  * accordingly */ 
...
/* @param  - [Input] : certASCII = the b64 encoded NULL terminated            */
  * certificate */ 
 /* @return - success : true                                                   */ 
 /* - failure : false                                                  */ 
 /* */ 
...
cleanup:
    ...
    certDER -= certLen; /* Remember d2i forwarded this, so set it
                                 * back */
...
bool ssl_remove_cert_from_store(const char *storePath, const char *searchThumb, const char *keyPath, const char *password)
{
    ...
    if (NULL != pem)
    free(pem);
    pem = calloc(len, sizeof(*pem));
                //get some memory to store the eventual PEM file 
    if (NULL == pem) {
    log_error("%s::%s(%d) : Out of memory", LOG_INF);
    goto cleanup;
    ...
    memcpy(pem, pemList->items[j]->cert, len);
                    //Copy the single string 
