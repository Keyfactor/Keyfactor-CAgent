...
ManagementConfigResp_t *ManagementConfigResp_fromJson(char *jsonString) {
    ...
    if (jsonString) {
        ...
        if (jsonRoot) {
            ...
            if (!resp) {
                log_error("%s::%s(%d) : Null pointer dereference - failed to "
                          "allocate ManagementConfigResp_t",
                          LOG_INF);
                json_delete(jsonRoot);
                return NULL;
            }
            ...
            if (jsonJob) {
                ...
                if (propString &&
                    ((jsonProps = json_decode(propString)) != NULL)) {
 ...
 *
 * @param[in] jsonString JSON string to parse
 * @return Pointer to newly allocated ManagementCompleteResp_t, or NULL on
 * failure
 */
ManagementCompleteResp_t *ManagementCompleteResp_fromJson(char *jsonString) {
    ...
    if (jsonString) {
        ...
        if (jsonRoot) {
            ...
            if (!resp) {
                log_error("%s::%s(%d) : Null pointer dereference - failed to "
                          "allocate ManagementCompleteResp_t",
                          LOG_INF);
                json_delete(jsonRoot);
                return NULL;
            }
...
InventoryCurrentItem_fromJsonNode(JsonNode *node) {
    ...
    if (node) {
        ...
        if (!result) {
            log_error("%s::%s(%d) : Null pointer dereference - failed to "
                      "allocate InventoryCurrentItem_t",
                      LOG_INF);
            return NULL;
        }
...
InventoryConfigResp_t *InventoryConfigResp_fromJson(char *jsonString) {
    ...
    if (jsonString) {
        ...
        if (jsonRoot) {
            ...
            if (!resp) {
                log_error("%s::%s(%d) : Null pointer dereference - failed to "
                          "allocate InventoryConfigResp_t",
                          LOG_INF);
                json_delete(jsonRoot);
                return NULL;
            }
            ...
            if (jsonJob) {
                ...
                if (jsonInv) {
                    ...
                    resp->Job.Inventory =
                        calloc(invCount, sizeof(InventoryCurrentItem_t *));
                    if (!resp->Job.Inventory) {
                        log_error("%s::%s(%d) : Null pointer dereference - "
                                  "failed to allocate Inventory array",
                                  LOG_INF);
                        json_delete(jsonRoot);
...
char *InventoryUpdateReq_toJson(InventoryUpdateReq_t *req) {
    ...
    if (req) {
        ...
        for (int i = 0; i < req->Inventory.count; ++i) {
            json_append_element(jsonInv, InventoryUpdateItem_toJsonNode(
                                             req->Inventory.items[i]));
        }
...
InventoryUpdateResp_t *InventoryUpdateResp_fromJson(char *jsonString) {
    ...
    if (jsonString) {
        ...
        if (jsonRoot) {
            ...
            if (!resp) {
                log_error("%s::%s(%d) : Null pointer dereference - failed to "
                          "allocate InventoryUpdateResp_t",
                          LOG_INF);
                json_delete(jsonRoot);
                return NULL;
            }
...
EnrollmentConfigResp_t *EnrollmentConfigResp_fromJson(char *jsonString) {
    ...
    if (jsonString) {
        ...
        if (jsonRoot) {
            ...
            if (!resp) {
                log_error("%s::%s(%d) : Null pointer dereference - failed to "
                          "allocate EnrollmentConfigResp_t",
                          LOG_INF);
                json_delete(jsonRoot);
                return NULL;
            }
            ...
            if (resp->Properties &&
                (jsonProps = json_decode(resp->Properties))) {
                log_verbose("%s::%s(%d) : \"Properties\": %s", LOG_INF,
                            resp->Properties);
                ...
                bool theBool = json_get_member_bool(
                    jsonProps, "separatePrivateKey", false);
#else
                bool theBool = false;
                char *theBoolString =
                ...
                log_verbose("%s::%s(%d) : Separate Private Key = %s", LOG_INF,
                            theBool ? "true" : "false");
                if (theBool) {
                    resp->PrivateKeyPath =
                        json_get_member_string(jsonProps, "privateKeyPath");

                    log_verbose("%s::%s(%d) : privateKeyPath: %s", LOG_INF,
                                resp->PrivateKeyPath);
                } else
                    resp->PrivateKeyPath = NULL;
            }
...
EnrollmentEnrollResp_t *EnrollmentEnrollResp_fromJson(char *jsonString) {
    ...
    if (jsonString) {
        ...
        if (jsonRoot) {
            ...
            if (!resp) {
                log_error("%s::%s(%d) : Null pointer dereference - failed to "
                          "allocate EnrollmentEnrollResp_t",
                          LOG_INF);
                json_delete(jsonRoot);
                return NULL;
            }
 ...
 *
 * @param[in] jsonString JSON string to parse
 * @return Pointer to newly allocated EnrollmentCompleteResp_t, or NULL on
 * failure
 */
EnrollmentCompleteResp_t *EnrollmentCompleteResp_fromJson(char *jsonString) {
    ...
    if (jsonString) {
        ...
        if (jsonRoot) {
            ...
            if (!resp) {
                log_error("%s::%s(%d) : Null pointer dereference - failed to "
                          "allocate EnrollmentCompleteResp_t",
                          LOG_INF);
                json_delete(jsonRoot);
                return NULL;
            }
