...
static int send_fetchlogs_job_complete(const char* sessionToken, const char* jobId, const char* endpoint, 
    int jobStatus, long auditId, const char* message, const char* log, struct CommonCompleteResp** pComp)
{
    char* url = NULL;
    ...
    if (sessionToken)
    {
        req->SessionToken = strdup(sessionToken);req->SessionToken = strdup(sessionToken);
    }
    else
    {
    ...
    int res = http_post_json(url, ConfigData->Username, ConfigData->Password, \
        ...
        ConfigData->AgentKeyPassword, jsonReq, &jsonResp, ConfigData->httpRetries, \
        ConfigData->retryInterval);
...
int cms_job_fetchLogs(struct SessionJob* jobInfo, char* sessionToken)
{
    ...
    log_info("%s::%s(%d) : Starting fetch logs job %s", LOG_INF, jobInfo->JobId);

    res = get_fetchlogs_config(sessionToken, jobInfo->JobId, jobInfo->ConfigurationEndpoint, &fetchLogsConf);
    ...
    if(res == 0 && fetchLogsConf && AgentApiResult_log(fetchLogsConf->Result, &statusMessage, &status))
    {
        ...
        {
			log_verbose("%s::%s(%d) : Audit Id: %ld", LOG_INF, fetchLogsConf->AuditId);
            char* log = NULL;
            ...
            status = STAT_SUCCESS;
            if( 0 != get_logs(ConfigData->LogFile, fetchLogsConf->MaxCharactersToReturn, &log) )
            {
                status = STAT_ERR;
            }
